...
export type DenemeSubParams =
  | { section: 'list' }
  | { section: 'create' }
  | { section: 'placement' }
  | { section: 'hub' };

export const DENEME_SUBMENU: { key: string; label: string; params: DenemeSubParams }[] = [
  { key: 'list', label: 'Kayıtlı denemeler', params: { section: 'list' } },
  { key: 'create', label: 'Deneme sonucu ekle', params: { section: 'create' } },
  {
    key: 'placement',
    label: 'Sınıf belirleme / deneme listesi',
    params: { section: 'placement' },
  },
];

export function denemeSubActiveKey(params?: DenemeSubParams | object | null): string | null {
  if (!params || typeof params !== 'object') return null;
  const p = params as Partial<DenemeSubParams>;
  if (p.section === 'create') return 'create';
  if (p.section === 'placement') return 'placement';
...
type Props = {
  ...
  onNavigate: (params: DenemeSubParams) => void;
};

/** Kenar menüde Denemeler + alt başlıklar */
export function DenemeNavItem({ focused, compact, activeKey, onNavigate }: Props) {
